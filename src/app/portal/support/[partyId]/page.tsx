...
function ChatPageComponent({ params }: { params: Promise<{ partyId: string }> }) {
    ...
    useEffect(() => {
        ...
        const unsubThread = subscribeToChatThread(partyId, (newThread) => {
            setThread(newThread);
            // Mark messages as read when the thread updates
            if (newThread) {
...
export default function ChatPage({ params }: { params: Promise<{ partyId: string }> }) {
    return (
        <Suspense fallback={<div className="flex justify-center items-center h-screen"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div>}>
            <ChatPageComponent params={params} />
        </Suspense>
    )
}
