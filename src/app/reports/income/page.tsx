...
import { Loader2, Printer, Share2, TrendingUp } from 'lucide-react';
...
export default function IncomeReport() {
  ...
  const { filteredTransactions, totalIncome } = useMemo(() => {
    const incomeTransactions = transactions.filter(t => t.type === 'income' || t.type === 'credit_income');
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <CardTitle className="flex items-center gap-2"><TrendingUp className="text-green-600" />Income Report</CardTitle>
              <CardDescription>A detailed report of all recorded income.</CardDescription>
            </div>
                ...
                {loading ? (
                  ...
                  filteredTransactions.map(t => (
                    <TableRow key={t.id}>
                      ...
                      <TableCell><Badge variant={t.type === 'credit_income' ? 'secondary' : 'outline'}>{t.type === 'credit_income' ? 'Credit Income' : 'Income'}</Badge></TableCell>
                  ...
                  <TableRow><TableCell colSpan={6} className="h-24 text-center">No income found for the selected criteria.</TableCell></TableRow>
                )}
              </TableBody>
               <TableFooter>
                <TableRow>
                    <TableCell colSpan={5} className="text-right font-bold">Total Income</TableCell>
                    <TableCell className="text-right font-bold font-mono text-green-600">{formatAmount(totalIncome)}</TableCell>
                </TableRow>
              </TableFooter>
            </Table>
          </div>
