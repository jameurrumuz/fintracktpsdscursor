...
const PaymentReceiptDialog = React.forwardRef<HTMLDivElement, PaymentReceiptDialogProps>(
  ({ isOpen, onOpenChange, transaction, party, appSettings, accounts = [], allTransactions }, ref) => {
    ...
    const generateSmsMessage = (lang: 'en' | 'bn', includeBalance: boolean): string => {
        ...
        const amountStr = formatAmount(transaction.amount, false);
        ...
        if (lang === 'bn') {
            ...
            baseMessage = `প্রিয় ${party.name}, ${verb} ${amountStr}। তারিখ: ${formatDate(transaction.date)}। বিবরণ: ${transaction.description}।`;
        } else {
    ...
    const previousBalance = transaction.previousBalance ?? 0;
    const isReceive = transaction.type === 'receive' || (transaction as any).paidAmount > 0;
    
    // Use paidAmount if it exists (from POS overpayment)
    const transactionAmount = (transaction as any).paidAmount || transaction.amount;
    
    // Calculate currentBalance correctly based on Bill vs Paid
    let currentBalance;
    if (transaction.type === 'sale' || transaction.type === 'credit_sale') {
        currentBalance = previousBalance + (transaction.amount || 0) - transactionAmount;
    } else {
        currentBalance = previousBalance + getPartyBalanceEffect(transaction, false);
    }
        ...
        <DialogContent className="max-w-xl">
          ...
          <div className="overflow-y-auto">
            <div ref={receiptContentRef} className="p-6 space-y-6 bg-white text-black text-sm">
              ...
              <section>
                 <p>
                  {isReceive ? 'Received with thanks from' : 'Paid to'} <span className="font-bold">{party?.name || (transaction as any).partyName || 'N/A'}</span>
